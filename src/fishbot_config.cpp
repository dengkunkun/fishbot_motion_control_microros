...
void FishBotConfig::init_http_server()
{
    static AsyncWebServer server(80);
    // Route to set parameters
    server.on("/set", HTTP_GET, [this](AsyncWebServerRequest *request)
        ...
        if (request->hasParam("key") && request->hasParam("value")) {
            ...
            if (this->config(key, value)) {
                request->send(200, "text/html", "<html><body><script>window.location.href = '/';</script></body></html>");
            } else {
    ...
    // Route to get current configuration and provide a form to set parameters
    server.on("/", HTTP_GET, [this](AsyncWebServerRequest *request)
              {
        String html = "<html><body>";
        html += "<h1>Current Configuration</h1>";
        html += "<pre>" + this->config_str() + "</pre>";
        html += "<h2>Set Parameter</h2>";
